...
import 'package:logistics_app/pages/news/logistics_page/logistics_detail_page.dart';
...
@AppRoute(path: 'logistics_list_page', name: '后勤动态列表页')
class LogisticsListPage extends StatefulWidget {
  const LogisticsListPage({super.key});
  @override
...
class _LogisticsListPageState extends State<LogisticsListPage>
  ...
  void initState() {
    ...
    super.initState();
    getLogisticsModelList(true);
  }

  Future<void> getLogisticsModelList(bool isRefresh) async {
    if (isRefresh) {
      _page = 1;
      _list = [];
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(S.of(context).logistics, style: TextStyle(fontSize: 16.px)),
        centerTitle: true,
      ...
      body: SafeArea(
        child: SmartRefreshWidget(
          ...
          enablePullUp: true,
          onRefresh: () {
            getLogisticsModelList(true).then((value) {
              _refreshController.refreshCompleted();
            });
          },
          onLoading: () {
            getLogisticsModelList(false);
          },
