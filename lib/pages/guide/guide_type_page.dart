...
class _GuideTypePageState extends State<GuideTypePage> {
  ...
  Future<void> openWordFile(String fileUrl) async {
    try {
      // 提取文件名
      String fileName =
          path.basename(fileUrl); // 获取 "IT每日一题2024.1.16-2025.2.15.docx"

      // 获取本地存储目录
      Directory tempDir = await getTemporaryDirectory();
      String filePath = '${tempDir.path}/$fileName'; // 拼接完整路径
      ...
      success: (data) {
        ...
        setState(() {
          if (guideTypeDetail?.file != null) {
            fileList = guideTypeDetail?.file
                    ?.split(',')
                    .map((e) =>
                        FileModel(fileName: path.basename(e), fileUrl: e))
                    .toList() ??
                [];
          }
        });
